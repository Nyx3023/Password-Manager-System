    ...
    <section className="settings-group desktop-lan-panel">
      ...
      {running && (
        <div className="desktop-lan-addresses">
          <p className="label-mono">LAN ADDRESSES</p>
          <p className="muted small desktop-lan-addr-hint">
            Use the one matching your Wi-Fi router. Skip virtual adapters
            (WSL, vEthernet, Hyper-V, Docker).
          </p>
          {(st?.addresses?.length ?? 0) === 0 && (
            <p className="muted small">No network interfaces detected.</p>
          )}
          {st?.addresses?.map((addr) => (
            <div
              key={`${addr.label}-${addr.ip}`}
              className={`desktop-lan-addr${addr.virtual ? " desktop-lan-addr--virtual" : ""}${
                addr.privateRange ? " desktop-lan-addr--lan" : ""
              }`}
            >
              <div className="desktop-lan-addr-info">
                <span className="desktop-lan-value">{addr.address}</span>
                <span className="muted small">
                  {addr.label}
                  {addr.virtual ? " (virtual)" : ""}
                  {!addr.virtual && addr.privateRange ? " (LAN)" : ""}
                </span>
              </div>
              <button
                type="button"
      ...
      {running && (
        <div className="desktop-lan-code-block">
          ...
          <p className="muted small desktop-lan-code-hint">
            Code expires in about 10 minutes. Enter it on the phone under
            Settings - Sync with PC.
          </p>
        </div>
      )}
